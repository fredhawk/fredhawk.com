...
Modals are these seemingly floating things that magically just take over the screen when taking a certain action. How does that happen? What does it look like on mobile? Is it good for mobile? When is it appropriate to use? Can you animate it?

### What considerations to think about?

There are a few things to think about when it comes to modals. When are they a good option? They can be used any time you want the users' attention focused on one thing. For example showing a lightbox for images or a dialog box confirming that the user really wants to cancel their flight or save their changes to a document. You get the drift.

We also see them in context of call to actions to sign up for newsletters and similar. I personally do not consider that a good practice but I understand the sentiment of it.
...
As it turns out there is a brilliant solution to that which I from now on will encourage on the [w3.org](http://w3.org)'s own accessibility site on the subject of dialogs and modals. There it is suggested that on mobile to turn it into a box that acts like any other. So no box in middle with an overlay but a full site instead. This makes it look less like a modal and more like a new page on mobile which is a nicer experience.
...
This adds accessibility to the modal and tells a screen reader that whatever is in the heading is what this modal is about. It also tells the screen reader that it is a modal.
...
Now that we can open and close it we can tie it all together with some CSS to make it actually work.
...
That is all we really need for it to work, but we are going to add some other base styles simply for visual purposes. To make it look better so to speak.
...
There are also versions of it for [Vue](https://github.com/theKashey/vue-focus-lock) and [ReactJs](https://github.com/theKashey/react-focus-lock) at the bottom of the npm link.

What I learned from this is that there is more to it than just a box showing up magically. Especially if we want this to be accessible.

And there you have it. That is all we need to make a modal that is accessible and usable. I hope you enjoyed it.

If there is anything you think that is missing and that should be added or changed please let me know, so I can update it.
